...
{{
  ...
  "plan": [
    ...
    async def generate_quiz(self, topic: str) -> Dict[str, Any]:
        """
        Generates a rigorous multiple-choice quiz based on the provided topic.
        """
        system_prompt = (
            "You are an expert educator and assessment creator. Create a challenging multiple-choice quiz about the given topic. "
            "QUESTIONS: Provide 5-10 distinct, high-quality questions. "
            "OPTIONS: For each question, provide exactly 4 options. "
            "CORRECTNESS: Ensure one and only one option is correct. Indicate the index of the correct option (0-3). "
            "Respond ONLY with a JSON object containing: "
            "{'questions': [{'question': 'string', 'options': ['string', 'string', 'string', 'string'], 'correct': int}]}"
        )
        ...
        logger.info("Generating quiz", topic=topic[:50])
        return await groq_provider.chat_completion(messages=messages, model=settings.GROQ_HIGH_CAPACITY_MODEL)
    ...
    async def generic_feature_handler(self, feature_type: str, content: str, context: dict = None) -> Dict[str, Any]:
        ...
        elif feature_type == "quiz-generator":
            return await self.generate_quiz(content)
